...
export const SearchInstructorScheduleInfo: React.FC<SearchInstructorScheduleInfoProps> = ({
  instructorId,
  openFlag,
  closeAction,
}) => {
  ...
  const filteredScheduleData = scheduleData?.filter(
    ...
    schedule.tentative_details?.description?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    schedule.tentative_details?.leadName?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    schedule.tentative_details?.paid_info?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    schedule.tentative_details?.phone?.includes(searchQuery) ||
    schedule.tentative_details?.pickup_location?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const getTimeAgo = (day?: string, start_time?: string) => {
    ...
    return formatDistanceToNow(new Date(parsedDate), { addSuffix: true, addSeconds: true  });
  };
                    ...
                    <div className="grid flex-1 grid-cols-1 gap-2 md:grid-cols-3">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {schedule.tentative_details?.pickup_location || "No area specified"}
                        </p>
                      </div>
                      <div>
                        <p className="text-sm">
