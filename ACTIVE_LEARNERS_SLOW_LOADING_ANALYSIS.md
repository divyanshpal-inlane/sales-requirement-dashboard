...
const demoLearnerIds = new Set(
  enrollmentData.filter((e: any) => e.progress?.type === "demo")
...
enrollmentData.forEach((e: any) => {
  const total = e.Courses?.total_lessons || 
                e.Courses?.duration || 
                e.progress?.total_hours || 10;
  learnerTotalLessons[e.learner_id] = Math.max(
    learnerTotalLessons[e.learner_id] || 0, total
  );
});
...
learnersWithSchedules.forEach((learner: any) => {
  ...
  const completedCount = learner.schedules?.filter(
  ...
  learner.hasTopupPending = learner.schedules?.some(
